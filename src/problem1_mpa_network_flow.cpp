...
/**
 * Create sample MPA instance for testing
 */
MPANetworkFlow createSampleInstance() {
    MPANetworkFlow mpa;

    // Add spawning sites
    mpa.addSpawningSite("S1", 100, 10);
    mpa.addSpawningSite("S2", 150, 15);
    mpa.addSpawningSite("S3", 80, 8);

    // Add settlement sites
    mpa.addSettlementSite("H1", 120, 12);
    mpa.addSettlementSite("H2", 100, 10);
    mpa.addSettlementSite("H3", 90, 9);

    // Add ocean current connections
    mpa.addCurrentConnection("S1", "H1", 60);
    mpa.addCurrentConnection("S1", "H2", 50);
    mpa.addCurrentConnection("S2", "H1", 80);
    mpa.addCurrentConnection("S2", "H3", 70);
    mpa.addCurrentConnection("S3", "H2", 60);
    mpa.addCurrentConnection("S3", "H3", 40);

    return mpa;
}

int main() {
    cout << "Marine Protected Area Network Flow Problem" << endl;
    cout << string(60, '=') << endl;

    MPANetworkFlow mpa = createSampleInstance();
    double budget = 50;
    ...
    auto [prot_spawn, prot_settle, max_flow] = mpa.greedySiteSelection(budget);

    cout << "Protected Spawning Sites: [";
    bool first = true;
    for (const auto& site : prot_spawn) {
        if (!first) cout << ", ";
        cout << site;
        first = false;
    }
    ...
    cout << "Protected Settlement Sites: [";
    first = true;
    for (const auto& site : prot_settle) {
        if (!first) cout << ", ";
        cout << site;
        first = false;
    }
    ...
    double cost = mpa.calculateTotalCost(prot_spawn, prot_settle);
    cout << "Total Cost: $" << fixed << setprecision(2) << cost << endl;
    cout << "Maximum Larval Flow: " << fixed << setprecision(0) << max_flow << endl;
