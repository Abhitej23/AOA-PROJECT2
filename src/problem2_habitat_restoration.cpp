...
/**
 * Create sample habitat restoration instance
 */
HabitatRestorationProblem createSampleInstance() {
    HabitatRestorationProblem problem;

    problem.addRestorationSite("Site_A",
        {"SeaTurtle", "Coral_A", "Grouper", "Seahorse"}, 25.0);

    problem.addRestorationSite("Site_B",
        {"SeaTurtle", "Dolphin", "Coral_B", "Mangrove"}, 30.0);

    problem.addRestorationSite("Site_C",
        {"Grouper", "Coral_A", "Coral_C", "Sponge"}, 20.0);

    problem.addRestorationSite("Site_D",
        {"Dolphin", "Whale", "Seahorse", "Coral_B"}, 35.0);

    problem.addRestorationSite("Site_E",
        {"Mangrove", "Coral_C", "Sponge", "Oyster"}, 18.0);

    problem.addRestorationSite("Site_F",
        {"Whale", "Oyster", "SeaTurtle", "Coral_A"}, 28.0);

    return problem;
}

int main() {
    cout << "Marine Habitat Restoration Planning Problem" << endl;
    cout << "(Reduces to Budgeted Maximum Coverage - NP-Complete)" << endl;
    cout << string(70, '=') << endl;

    HabitatRestorationProblem problem = createSampleInstance();
    double budget = 60.0;

    cout << "\nTotal species in ecosystem: " << problem.getAllSpecies().size() << endl;
    cout << "Species: [";
    bool first = true;
    for (const auto& sp : problem.getAllSpecies()) {
        if (!first) cout << ", ";
        cout << sp;
        first = false;
    }
    ...
    cout << "\nNumber of restoration sites: " << problem.getSites().size() << endl;
    cout << "Budget: $" << budget << endl;

    cout << "\n" << string(70, '-') << endl;
    cout << "Site Details:" << endl;
    cout << string(70, '-') << endl;
    for (int i = 0; i < problem.getSites().size(); i++) {
        const auto& site = problem.getSites()[i];
        cout << i << ". " << site.id << ": $" << (int)site.cost
             << " - Covers " << site.species.size() << " species: [";
        first = true;
        for (const auto& sp : site.species) {
            if (!first) cout << ", ";
            cout << sp;
            first = false;
        }
    ...
    cout << "\n" << string(70, '=') << endl;
    cout << "GREEDY SOLUTION (Cost-Effectiveness Ratio)" << endl;
    cout << string(70, '=') << endl;
    auto [selected, coverage, cost] = problem.greedyCoverage(budget);

    cout << "Selected sites: [";
    first = true;
    for (int idx : selected) {
        if (!first) cout << ", ";
        cout << problem.getSites()[idx].id;
        first = false;
    }
    cout << "]" << endl;
    cout << "Total cost: $" << fixed << setprecision(2) << cost << endl;
    cout << "Species covered: " << coverage << " / "
         << problem.getAllSpecies().size() << endl;
    cout << "Coverage percentage: " << fixed << setprecision(1)
         << (100.0 * coverage / problem.getAllSpecies().size()) << "%" << endl;

    // Simple greedy solution
    cout << "\n" << string(70, '=') << endl;
    cout << "GREEDY SOLUTION (Maximum Marginal Coverage)" << endl;
    cout << string(70, '=') << endl;
    auto [selected2, coverage2, cost2] = problem.greedyCoverageSimple(budget);
    ...
    cout << "]" << endl;
    cout << "Total cost: $" << fixed << setprecision(2) << cost2 << endl;
    cout << "Species covered: " << coverage2 << " / "
         << problem.getAllSpecies().size() << endl;
    cout << "Coverage percentage: " << fixed << setprecision(1)
         << (100.0 * coverage2 / problem.getAllSpecies().size()) << "%" << endl;

    // Optimal solution (small instance only)
    cout << "\n" << string(70, '=') << endl;
    cout << "OPTIMAL SOLUTION (Exhaustive Search)" << endl;
    cout << string(70, '=') << endl;
    auto [opt_selected, opt_coverage, opt_cost] = problem.optimalSolutionSmall(budget);

    cout << "Selected sites: [";
    first = true;
    for (int idx : opt_selected) {
        if (!first) cout << ", ";
        cout << problem.getSites()[idx].id;
        first = false;
    }
    cout << "]" << endl;
    cout << "Total cost: $" << fixed << setprecision(2) << opt_cost << endl;
    cout << "Species covered: " << opt_coverage << " / "
         << problem.getAllSpecies().size() << endl;
    cout << "Coverage percentage: " << fixed << setprecision(1)
         << (100.0 * opt_coverage / problem.getAllSpecies().size()) << "%" << endl;

    double approx_ratio = (opt_coverage > 0) ? (double)coverage / opt_coverage : 1.0;
    cout << "\nGreedy approximation ratio: " << fixed << setprecision(3)
         << approx_ratio << endl;

    // Demonstrate NP-completeness
    problem.demonstrateNPCompleteness();
